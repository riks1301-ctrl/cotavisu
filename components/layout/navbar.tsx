      ...
      <div className={layout.container}>
        <div className="flex h-20 items-center justify-between lg:h-[5.5rem]">
          <Link href="/" className="flex items-center gap-3">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-blue-600 shadow-md lg:h-14 lg:w-14">
              <Eye className="h-7 w-7 text-white lg:h-8 lg:w-8" />
            </div>
          ...
          <nav className="hidden items-center gap-10 md:flex">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className={`${type.nav} py-2 transition-colors ${
                  pathname.startsWith(link.href) ? "font-semibold text-blue-600" : "text-gray-700 hover:text-gray-900"
                }`}
              >
                {link.label}
              </Link>
          ...
          <div className="hidden items-center gap-4 md:flex">
            {user ? (
              <>
                ...
                {profile?.role === "buyer" && (
                  <ButtonLink href="/pedidos/novo" size="lg" className="shadow-md">
                    <Plus className="mr-2 h-5 w-5" /> Criar pedido
                  </ButtonLink>
                )}
                {profile?.role === "supplier" && (
                  <ButtonLink href="/pedidos" size="lg" className="shadow-md">
              ...
              <>
                ...
                <ButtonLink href="/cadastro" size="lg" className="shadow-md">Cadastrar grátis</ButtonLink>
              </>
            )}
          </div>

          <button className="rounded-xl p-3 md:hidden" onClick={() => setOpen(!open)} aria-label="Menu">
            ...
            <nav className="flex flex-col gap-5">
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className={`${type.nav} py-2 text-gray-800`}
              ...
              <div className="flex flex-col gap-4 border-t pt-6">
                {user ? (
                  ...
                  <>
                    ...
                    <ButtonLink href="/cadastro" className="w-full" size="lg">Cadastrar grátis</ButtonLink>
                  </>
                )}
              </div>
            </nav>
